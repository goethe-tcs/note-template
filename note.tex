...
% If this document is written mainly in german:
\usepackage[english,ngerman,shorthands=off]{babel}
% If this document is written mainly in english:
...
\begin{itemize}
  ...
  \selectlanguage{english}\enquote{Quotation marks}\selectlanguage{ngerman}
...
Zwischen dem \verb|\cite|-Kommando und dem Namen sollte hierbei unbedingt ein \selectlanguage{english}non-breaking space\selectlanguage{ngerman} \verb|~| stehen, damit die Zeile dort nicht plötzlich endet. Also so: \verb|Karp~\cite{DBLP:conf/coco/Karp72}|. Falls der Name mehrmals hintereinander erwähnt wird, sollte das \verb|\cite| Kommando nur beim ersten Auftauchen benutzt werden.
